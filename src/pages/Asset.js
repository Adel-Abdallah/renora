...
export default function Assets() {
  ...
  <table className="mt-8 w-full table sm:px-2">
    <thead>
    ...
    <tbody className="bg-white divide-gray-200 divide-y-4">
      {data.map((asset) => (
        <tr
          ...
          style={{ height: "80px" }}
        >
          <td className="px-2 py-2 whitespace-nowrap font-medium text-gray-900 bg-slate-50 shadow-indigo-500/40" style={{width:'60px'}}>
...
{isMobile && (
  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
   {data.map((asset) => (
  <div key={asset.asset} className="bg-white shadow-md border border-gray-200 my-4 space-y-16 rounded p-4">
    <div className="flex items-center">
      <div className="w-10 h-10 rounded-full overflow-hidden mr-2 bg-slate-200">
        <img className="w-full h-full object-cover bg-slate-200" src={asset.logo} alt="logo" />
      </div>
      <div>
